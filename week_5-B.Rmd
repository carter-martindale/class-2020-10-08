...
draws2 <- draws %>% 
  pivot_longer(cols = 1:51, names_to = "state",
...
draws2 %>% 
  ...
  geom_density() +
  labs(y = "Density", x = "Trump's Share of the Votes",
       title = "Density Plot of Trump's Vote Share")
...
First, repeat Scene 2 to make a column called `oh_win` that uses a `map` function to figure out whether Trump wins Ohio in each of these simulated elections. Then, create a column `called` both that is TRUE if Trump wins both PA and OH and FALSE otherwise.
...
elections %>% 
  ...
  mutate(both = if_else(condition = pa_win == TRUE &
                          oh_win == TRUE,
         true = TRUE,
         false = FALSE)) %>% 
  summarise(avg_pa_win = mean(pa_win),
            avg_oh_win = mean(oh_win),
            avg_both_win = mean(both))
...
The Economist says that results in PA and OH are likely to be highly correlated - around 0.7. 
